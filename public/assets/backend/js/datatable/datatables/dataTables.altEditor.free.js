...
(function (factory) {
    ...
    $.extend(altEditor.prototype, {
        ...
        _constructor: function () {
            ...
            that.closeModalOnSuccess =
                dt.settings()[0].oInit.closeModalOnSuccess;
        ...
        _setup: function () {
            ...
            var modal =
                '<div class="modal fade altEditor-modal reveal" id="' +
                modal_id +
                '" tabindex="-1" role="dialog" data-reveal>' +
                '<div class="modal-dialog">' +
                '<div class="modal-content">' +
                '<div class="modal-header">' +
                '<h4 style="padding-top: 1rem;padding-left: 1rem;" class="modal-title"></h4>' +
                '<button style="margin: initial;" type="button" class="close close-button" data-dismiss="modal" data-close aria-label="' +
            ...
            $(this.modal_selector).on("input", "[data-unique]", function (elm) {
                if (
                    $(elm.target).attr("data-unique") == null ||
                    $(elm.target).attr("data-unique") === "false"
                ) {
                    return;
                }
                var target = $(elm.target);
                var colData = dt
                    .column("th:contains('" + target.attr("name") + "')")
                    .data();
                ...
                for (var j in colData) {
                    ...
                    if (
                        target.val() == colData[j] &&
                        colData[j] != selectedCellData
                    ) {
                        elm.target.setCustomValidity(
        ...
        _initLanguage: function () {
            ...
            this.language.error = {
                message:
                    this.language.error.message ||
                    "There was an unknown error!",
                label: this.language.error.label || "Error!",
                responseCode:
                    this.language.error.responseCode || "Response code: ",
        ...
        _openEditModal: function () {
            ...
            for (var j in columnDefs) {
                if (columnDefs[j].name != null) {
                    var jquerySelector =
                        "#" +
                        columnDefs[j].name.toString().replace(/\./g, "\\.");
                    ...
                    // Added Select2
                    if (
                        columnDefs[j].type.indexOf("select") >= 0 &&
                    ...
                    // Added checkbox
                        ...
                        if (
                            this._quoteattr(selectedValue) === "true" ||
                            this._quoteattr(selectedValue) == "1"
                        ) {
                            // MS SQL Databases use bits for booleans. 1 is equivlent to true, 0 is false
                            $(selector)
                                .find(jquerySelector)
                                .prop(
                                    "checked",
                                    this._quoteattr(selectedValue)
                        ...
                        if (columnDefs[j].dateFormat !== "") {
                            ...
                            if (mDate && mDate.isValid()) {
                                $(selector)
                                    .find(jquerySelector)
                                    .val(
                                        mDate.format(columnDefs[j].dateFormat)
                                    );
                            }
                        }
                    }
                    ...
                    $(selector)
                        ...
                        .val(selectedValue) // this._quoteattr or not? see #121
        ...
        _editRowData: function () {
            ...
            $inputs
                .filter(':input[type!="file"], :input[type="checkbox"]')
                .each(function (i) {
                    ...
                    if (that.encodeFiles) {
                        ...
                        that.getBase64(
                            ...
                            function (filecontent) {
                                rowDataArray[$(context).attr("id")] =
                                    filecontent;
                                --numFilesQueued;
                            }
                        );
                    } else {
                        rowDataArray[$(this).attr("id")] =
                            $(this).prop("files")[0];
                    }
                }
            });
        ...
        _openDeleteModal: function () {
            ...
            var fill = function () {
                var btns =
                    '<button type="button" data-content="remove" class="btn btn-default button secondary" data-close data-dismiss="modal">' +
                    that.language.modalClose +
                    "</button>" +
                    '<button type="submit"  data-content="remove" class="btn btn-danger button" id="deleteRowBtn">' +
                    that.language.delete.button +
                    "</button>";
                $(selector)
                    .find(".modal-title")
                    .html(that.language.delete.title);
                $(selector)
                    .find(".modal-body")
                    .html(
                        that.language.deleteMessage ||
                            `<h5>Are you sure you wish to delete ${adata.count()} rows?</h5>`
                    );
        ...
        completeColumnDefs: function () {
            ...
            for (var i in dt.context[0].aoColumns) {
                ...
                columnDefs[i] = {
                    ...
                    datetimepicker: obj.datetimepicker
                        ? obj.datetimepicker
                        : false,
                    editorOnChange: obj.editorOnChange
                        ? obj.editorOnChange
                        : null,
                    style: obj.style ? obj.style : "",
                    dateFormat: obj.dateFormat ? obj.dateFormat : "",
                    optionsSortByLabel: obj.optionsSortByLabel
                        ? obj.optionsSortByLabel
                        : false,
        ...
        createDialog: function (
            ...
            for (var j in columnDefs) {
                ...
                if (columnDefs[j].type.indexOf("hidden") >= 0) {
                    ...
                    if (columnDefs[j].inline) {
                        //to add upto 4 inline columns
                        if (count == 0) {
                            count++;
                            data +=
                                "<div style='margin-left: initial;margin-right: initial;' class='form-group row' id='alteditor-row-" +
                                this._quoteattr(columnDefs[j].name) +
                                "'>";
                            data +=
                    ...
                    if (columnDefs[j].type.indexOf("select") >= 0) {
                        ...
                        if (optionsArray.length > 0) {
                            ...
                            for (var i = 0; i < optionsArray.length; i++) {
                                options +=
                                    "<option value='" +
                                    this._quoteattr(optionsArray[i]) +
                                    "'>" +
                                    optionsArray[i] +
                                    "</option>";
                            }
                        ...
                        data +=
                            ...
                            (columnDefs[j].select2 ? " select2" : "") +
                            "' " +
                            fillAttrs(columnDefs[j], [
                                "name",
                                "style",
                                "readonly",
                                "disabled",
                    ...
                    // Adding text-inputs and error labels, but also new HTML5 types (email, color, ...)
                        ...
                        data +=
                            ...
                            this._quoteattr(columnDefs[j].style) +
                            "' " +
                    ...
                    if (
                        !columnDefs[j].inline ||
                        (+j + 1 < columnDefs.length &&
                            !columnDefs[+j + 1].inline)
                    ) {
            ...
            var fill = function () {
                var btns =
                    '<button type="button" data-content="remove" class="btn btn-default button secondary" data-dismiss="modal" data-close>' +
                    closeCaption +
                    "</button>" +
                    '<button type="submit" form="' +
                    formName +
                    '" data-content="remove" class="btn btn-primary button" id="' +
                    buttonClass +
                    '">' +
                    buttonCaption +
                    "</button>";
            ...
            for (var j in columnDefs) {
                if (columnDefs[j].select2) {
                    ...
                    $(selector)
                        .find("select#" + columnDefs[j].name)
                        .select2(columnDefs[j].select2);
                } else if (columnDefs[j].datepicker) {
                    // Require jquery-ui
                    $(selector)
                        .find("#" + columnDefs[j].name)
                        .datepicker(columnDefs[j].datepicker);
                ...
                if (columnDefs[j].editorOnChange) {
                    ...
                    $(selector)
                        .find("#" + $.escapeSelector(columnDefs[j].name))
                        .attr("alt-editor-id", this._quoteattr(j));
                    $(selector)
                        .find("#" + $.escapeSelector(columnDefs[j].name))
                        .on("change", function (elm) {
                            var f =
                                columnDefs[$(this).attr("alt-editor-id")]
                                    .editorOnChange;
                            f(elm, that);
                        });
                }
                ...
                if (columnDefs[j].type.indexOf("select") >= 0) {
                    if (columnDefs[j].optionsSortByLabel) {
                        ...
                        $(selector)
                            .find(jquerySelector)
                            .val($(jquerySelector + " option:first").val());
                    }
                }
            }
        },
        ...
        _errorCallback: function (response, status, more) {
            ...
            if (error.responseJSON) {
                ...
                errstr =
                    error.status == null
                        ? ""
                        : this.language.error.responseCode + error.status;
            }

            var message =
                '<div class="alert alert-danger" role="alert">' +
                "<strong>" +
                this.language.error.label +
                "</strong>" +
                (errstr ? "<br />" + errstr : "") +
                "</div>";
        ...
        internalOpenDialog(selector, onopen) {
            ...
            if ($sel.modal) {
                ...
                console.error(
                    "You must load Bootstrap or Foundation in order to open modal dialogs"
                );
                return;
            }
        },
